            ...
            if app in list(loan_apps_regex.keys()):
                ...
                while i < len(data):
                    ...
                    if data["category"][i] == "disbursed":
                        ...
                        individual_loan_details["messages"].append(
                            {"date": str(data['timestamp'][i]), "message": str(data['body'][i])})
                        loan_count += 1
                        ...
                        while j < len(data):
                            ...
                            if data["category"][j] == "due":
                                ...
                                if (due_date - disbursal_date).days < 16:
                                    ...
                                    individual_loan_details["messages"].append(
                                        {"date": str(data['timestamp'][j]), "message": str(data['body'][j])})
                                    k = j + 1
                                    while k < len(data):
                                        ...
                                        if data["category"][k] == "overdue":
                                            ...
                                            try:
                                                individual_loan_details['overdue_days'] = overdue_days_extract(msg3,
                                                                                                               app)
                                            except:
                                                pass
                                            individual_loan_details['overdue_check'] += 1
                                            individual_loan_details["messages"].append(
                                                {"date": str(data['timestamp'][k]), "message": str(data['body'][k])})
                                            m = k + 1
                                            while m < len(data):
                                                ...
                                                if data["category"][m] == "closed":
                                                    ...
                                                    individual_loan_details["messages"].append(
                                                        {"date": str(data['timestamp'][m]),
                                                         "message": str(data['body'][m])})
                                                    k = m + 1
                                                    FLAG = True
                                            ...
                                            if (due_date - disbursal_date).days <= 16:
                                                ...
                                                individual_loan_details["messages"].append(
                                                    {"date": str(data['timestamp'][k]),
                                                     "message": str(data['body'][k])})
                                            else:
                                                logger.info(
