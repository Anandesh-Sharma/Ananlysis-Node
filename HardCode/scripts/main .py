...
def preprocessing(loan_data, cust_id):
        ...
        if app == 'CASHBN' or app == 'KREDTB' or app == 'KREDTZ' or app == 'LNFRNT' or app == 'RRLOAN' or app == 'LOANAP' or app == 'KISSHT' or app == 'GTCASH' or app == 'FLASHO' or app == 'CSHMMA' or app == 'ZPLOAN':
            ...
            while i < len(grp):
                ...
                if is_disbursed(message):
                    ...
                    individual_loan_details['loan_disbursed_amount'] = disbursed_amount_extract(message)
                        ...
                        if is_disbursed(message_new):
                            ...
                            while k < len(grp):
                                ...
                                if is_overdue(message_overdue):
                                    ...
                                    overdue_first_date = datetime.strptime(str(grp['timestamp'][k]), '%Y-%m-%d %H:%M:%S')
                                    individual_loan_details['overdue_max_amount'] = overdue_amount_extract(grp, overdue_first_date)
                                    m = k + 1
                                    while m < len(grp):
                                        ...
                                        if is_closed(message_closed):
                                            ...
                                            individual_loan_details['loan_closed_amount'] = closed_amount_extract(message_closed)
                                            k = m + 1
                                    ...
                                    loan_duration = (closed_date - disbursed_date).days
                                    individual_loan_details['loan_duration'] = loan_duration
                                    individual_loan_details['loan_closed_amount'] = closed_amount_extract(message_overdue)
                                    break
                            ...
                            while m < len(grp):
                                ...
                                if is_closed(message_closed):
                                    ...
                                    loan_duration = (closed_date - disbursed_date).days
                                    individual_loan_details['loan_duration'] = loan_duration
                                    individual_loan_details['loan_amount'] = closed_amount_extract(message_closed)
                                    j = m + 1
                                    logger.info("Loan Closed!")
                                    break 
                                elif is_disbursed(message_closed):
                            ...
                            loan_duration = (closed_date - disbursed_date).days
                            individual_loan_details['loan_duration'] = loan_duration
                            individual_loan_details['loan_closed_amount'] = closed_amount_extract(message_new)
                            logger.info("Loan Closed!")
                            break 
                        j += 1       
...
'''df_main=pd.DataFrame(columns=['closed_date', 'disbursed_date', 'due_date', 'loan_closed_amount',
        'loan_disbursed_amount', 'loan_due_amount', 'loan_duration', 'overdue_max_amount', 'APP','cust_id'])
#list_user= pd.read_csv('/home/pc/Desktop/abhishek/workspace/UNIQUE CLIENTS.csv')
#list_user = list(list_user['user_id'])
list_user = [6013, 102425, 31873, 24276, 6000, 6010, 6018, 6028, 6023, 6031, 6038, 6041, 6049, 6050, 6053, 6059, 6064, 6069, 6073, 6078, 6087, 6091, 6097, 6104, 6107, 6287, 6387, 6558,
              6574, 6757, 6839, 6903, 7296, 7318, 7413, 7881, 7963, 9851, 9852, 9853]
print(list_user)
for cust_id in list_user:
    loan_data = get_customer_data(cust_id)
    df=pd.DataFrame(columns=['closed_date', 'disbursed_date', 'due_date', 'loan_closed_amount',
        'loan_disbursed_amount', 'loan_due_amount', 'loan_duration', 'overdue_max_amount', 'APP'])
    
    a = preprocessing(loan_data, cust_id)

    for i in a.keys():
        j=pd.DataFrame.from_dict(a[i]).T
        j['APP']=[i]*j.shape[0]
        try:
            j.columns=['closed_date', 'disbursed_date', 'due_date', 'loan_closed_amount',
            'loan_disbursed_amount', 'loan_due_amount', 'loan_duration', 'overdue_max_amount', 'APP']
            df =pd.concat([df, j])
            df.reset_index(inplace=True,drop=True)
        except:
            continue
    df['cust_id']=[cust_id]*df.shape[0]
    df_main=pd.concat([df_main, df])
    df_main.reset_index(inplace=True,drop=True)

#df_main.to_csv('main_new(1).csv')    
print(df_main)'''
