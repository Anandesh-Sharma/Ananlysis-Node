...
def find_info(user_id):
    ...
    if not user_data:
        ...
        db.update({'cust_id': int(user_id)}, {"$set": {'modified_at':str(datetime.now(pytz.timezone('Asia/Kolkata'))),
                                                       'parameters.AC_NO':'',
                                                       'parameters.balance_on_loan_date':-1,
                                                       'parameters.last_month_bal':-1,
                                                       'parameters.second_last_month_bal':-1,
                                                       'parameters.third_last_month_bal':-1,
                                                       'parameters.count_creditordebit_msg':-1,
                                                       'parameters.no_of_accounts':-1}}, upsert=True)
