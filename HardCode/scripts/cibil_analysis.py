...
def cibil_analysis(df,cibil_score,user_id):
  ...
  try:
    for i,row in df.iterrows():
      ...
      if row['written_amt_total'] != 0 and not np.isnan(row['written_amt_total']):
        amt_total = row['written_amt_total']
      if row['written_amt_principal'] != 0 and not np.isnan(row['written_amt_principal']):
          ...
          if amt_total==0 or np.isnan(amt_total):
            if amt_principal==0 or np.isnan(amt_principal):
              if payment_r in selected:
                ans=True
    return {'status': True, 'message': 'success','ans':ans}
